...
class BenchVstmiaDRange : public EntoBench::CaptureProblem<BenchVstmiaDRange, 32>
{
public:
  void prepare_impl()
  {
...
int main()
{
  ...
  BenchVstmiaDRange problem;
  ENTO_BENCH_HARNESS_TYPE(decltype(problem));
  BenchHarness harness(problem, "bench_vstmia_d_range");
  harness.run();
